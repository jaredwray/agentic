...
description: Roll out a hardened npm release pipeline — signer policy, signed release intent, stage-only trusted publishing, Drydock artifact review, verification gates — one improvement per PR across a strict 4-phase rollout, with status tracked in the target repo's DEFENSE_IN_DEPTH.md. Use when asked to harden npm publishing, set up trusted publishing or release signing, or secure the release pipeline for high-impact packages. Manual, resumable, one PR at a time.
...
**Summary:** use the best of both worlds — GitHub Actions and npm trusted publishing provide build/publish provenance, while a non-GitHub maintainer signature proves human release approval. A release proceeds only when at least one approved maintainer signs the exact release intent. GitHub is the build system, not the human root of trust. CI's publish authority is stage-only; a maintainer promotes with 2FA after [Drydock](https://drydock.org) reviews the staged artifact.
...
- [ ] `pnpm-workspace.yaml` security baseline added (`minimumReleaseAge`, `minimumReleaseAgeStrict`, `minimumReleaseAgeIgnoreMissingTime`, `blockExoticSubdeps`, `strictDepBuilds`, `dangerouslyAllowAllBuilds: false`, `trustPolicy: no-downgrade`, `allowBuilds: {}`; no first-party `minimumReleaseAgeExclude` / `trustPolicyExclude`)
...
- [ ] npm trusted publisher configured **stage-only** for the pilot (provider GitHub Actions, exact repo, workflow `publish.yml`, environment `npm-publish` — [reference.md § 16](./reference.md#16-npm-trusted-publishing-configuration)) (manual)
...
- [ ] Test release staged on a prerelease tag (`<pkg>@x.y.z-test.0`)
...
- [ ] After the staged flow works: npm package setting **Require two-factor authentication and disallow tokens** applied (manual)
...
- [ ] (Optional) Drydock Workflow Gate on the `npm-publish` environment ([reference.md § 15](./reference.md#15-github-environment-npm-publish)) (manual)
...
**Phase 3 — Pilot package.** Mixed. Auto items: package selection (recorded in `DEFENSE_IN_DEPTH.md`), publish workflow, signed release intent, staged test release, negative tests. Manual items: GitHub environment creation, stage-only trusted publisher config, Drydock connection and promotion, post-rollout npm 2FA setting and token revocation.
...
- [x] `sfw pnpm install --frozen-lockfile` succeeds
