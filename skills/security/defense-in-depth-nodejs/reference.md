...
- Rulesets are judged by their contents, not their name: `--check` validates enforcement, rule
  types, review count, last-push approval, targets, and the bypass list, and apply mode overwrites
  a same-name ruleset with the canonical config — a pre-existing weak ruleset can't pass as
  compliant.
