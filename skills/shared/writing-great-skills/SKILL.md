...
are manual-only; that staged publishing in skills uses `pnpm stage publish` (pnpm ≥ 11.3), not
the npm CLI; and that those tarball paths are prefixed with `./` and include `--no-git-checks`.
