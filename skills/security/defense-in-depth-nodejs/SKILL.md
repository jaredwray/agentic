...
2. **§ 2 CODEOWNERS and cloud bootstrap** — `.github/CODEOWNERS` including `/.vscode/`, `/.claude/`,
   and `/.codex/` (file PR); Aikido Safe Chain on Codespaces, Cursor Cloud Agents, and Claude Code on
   the web (file PR; skip without `pnpm-lock.yaml`); Codex cloud setup and Claude Code network
   settings `(manual)`; Dev Container `image` digest pin (file PR; skip without a
   `devcontainer.json`).
...
3. **Audit.**
   ...
   - Reconcile every checkbox against actual repo state per `security-status-tracking`. For § 5
     ...
     Skip the Safe Chain item and the Codex / Claude Code environment items when `pnpm-lock.yaml`
     is absent. Skip the Dev Container image-pin item when no `devcontainer.json` exists. Never
     silently uncheck a regression — stop and report it.
